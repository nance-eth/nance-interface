...
import {
  VOTING_POWER_QUERY,
  VOTING_POWER_WITHOUT_PROPOSAL_QUERY,
} from "./queries/Vote";
...
export default function useVotingPower(
  ...
  const { loading, data, error } = useQuery<{ vp: SnapshotVotingPower }>(
    VOTING_POWER_QUERY,
    {
...
export async function fetchVotingPowerWithoutProposal(
  voter: string,
  space: string
): Promise<number> {
  const ret = await fetch(`${SNAPSHOT_HUB}/graphql`, {
    ...
    headers: SNAPSHOT_HEADERS,
    body: JSON.stringify({
      query: VOTING_POWER_WITHOUT_PROPOSAL_QUERY,
      variables: { voter, space },
    }),
  }).then((res) => res.json());

  if (ret.errors) {
    console.warn(
      "fetchVotingPowerWithoutProposal errors occurred: ",
      ret.errors
    );
    return 0;
  } else {
    return ret.data.vp.vp;
  }
}
